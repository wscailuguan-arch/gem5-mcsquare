...
echo '=== MCSQ_INC (value right after sizeElided +=) ==='
grep MCSQ_INC $D/err.txt
echo '=== elide=1 packets at memctrl (with tick) ==='
grep 'elide=1' $D/err.txt | grep memctrl
echo '=== elide pseudo-inst + dumpstats ticks ==='
grep -e 'memcpy_elide(' -e dumpstats $D/fullout.txt | head -4
